...
# def set(img,label_1,label_2,g,fw):
#     """
#     train fine
#     """
#     label_coarse = tf.where(tf.equal(label_1, g))
#     img = tf.gather(img, label_coarse)
#     label_2 = tf.gather(label_2, label_coarse)-fw
#     return img,label_2
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        val_dataset = dataset_factory.get_dataset(
            FLAGS.dataset_name, "Clothes-test-g4-fine", FLAGS.dataset_dir)
        ######################
        ...
        with tf.control_dependencies(update_ops):
            with tf.device(deploy_config.optimizer_device()):
                net_opt = {}
                for i in range(FLAGS.num_networks):
                    net_opt["{0}".format(i)] = tf.train.AdamOptimizer(lr,
        ...
        with tf.device(deploy_config.inputs_device()):
            ...
            [image, label] = provider.get(['image', 'label_2'])
        ...
        ##############################################################
        # Create a val_dataset provider that loads data from the dataset #
        ##############################################################
        with tf.device(deploy_config.inputs_device()):
            examples_per_shard = 1024
            min_queue_examples = examples_per_shard * FLAGS.input_queue_memory_factor
            val_provider = slim.dataset_data_provider.DatasetDataProvider(
                ...
                common_queue_capacity=min_queue_examples + 3 * FLAGS.batch_size,
                common_queue_min=min_queue_examples)
            [val_image, val_label] = val_provider.get(['image', 'label_2'])
            # image,label=set(image,label_1,label_2,FLAGS.coarse,fw[FLAGS.coarse])
            ...
            val_labels = slim.one_hot_encoding(val_labels, dataset.num_classes)
            val_batch_queue = slim.prefetch_queue.prefetch_queue(
                [val_images, val_labels], capacity=16 * deploy_config.num_clones,
                num_threads=FLAGS.num_preprocessing_threads)
            ...
            val_images_splits = tf.split(axis=0, num_or_size_splits=FLAGS.num_gpus, value=val_images)
            val_labels_splits = tf.split(axis=0, num_or_size_splits=FLAGS.num_gpus, value=val_labels)
            ...
            with tf.device('/gpu:%d' % k):
                with tf.name_scope('tower_%d' % k) as scope:
                    with tf.variable_scope(tf.get_variable_scope()):
                        ...
                        _,net_end_point_val=network_fn["{0}".format(0)](val_images_splits[k],scope=('dmlnet_%d'%0))
                        val_pred=net_end_point_val["Predictions"]
                        val_truth=tf.argmax(val_labels_splits[k],axis=1)
                        # Reuse variables for the next tower.
                        tf.get_variable_scope().reuse_variables()

                        # Retain the summaries from the final tower.
                        summaries = tf.get_collection(tf.GraphKeys.SUMMARIES, scope)
                        summaries.append(tf.summary.image('images', images))
        ...
        # laststep = ckpt.model_checkpoint_path.split('/')[-1].split('-')[-1]
        # saver.restore(sess,model_file)
        # print(type(int(laststep)))
        loss_all=0
        precision_all=0
        best_acc = 0
        for step in range(0,FLAGS.max_number_of_steps):
            ...
            loss_all+= float(net_loss_value["{0}".format(0)])
            precision_all+=float(precision_value["{0}".format(0)])
            if step % 10 == 0:
                format_str = '%s: step %d, net0all_loss = %.2f, all_accuracy = %.4f'
                print(format_str % (FLAGS.dataset_name, step, loss_all/(step+1e-7),
                                  precision_all/(step+1e-7)))

            f = open("result_resnet18_g4ttt.txt", 'a+')
            if step % 11376 ==0:
                print("waiting validating...")
                num_iter = int(math.ceil(val_dataset.num_samples / FLAGS.batch_size))
                valstep=0
                count=0
                while valstep < num_iter:
                    valstep += 1
                    count+=sess.run(val_pre)
                    val_acc=count/valstep
                # if val_acc>best_acc:
                #     best_acc=val_acc
                #     saver.save(sess,'ckptmobilenet_finetune/bset.ckpt',global_step=step)
                lr = sess.run(tf.get_collection('learning_rate'))

                f.write("step %d------------->>>" % step)
                f.write("lr>>%.5f        " % lr[0])
                f.write("val_acc>>%.5f"%val_acc+"\n")
            if step % 100 == 0:
